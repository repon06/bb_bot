...
from helper.design import green, yellow
...
def open_perpetual_order(exchange, market_symbol, buy_price,
    ...
    try:
        ...
        max_order_size = market_info['limits']['amount']['max']

        order_amount = TRADE_AMOUNT / current_price
        if order_amount < min_order_size:
            logging.info(f"Слишком маленький ордер: {order_amount}, мин: {min_order_size}; max: {max_order_size}")
            return {}
...
def set_take_profits_perpetual(exchange, market_symbol, trade_type, order_amount, take_profits):
    ...
    for idx, tp_price in enumerate(take_profits):
        try:
            # !!!TODO: нужна проверка - чтобы ТП был не выше/ниже цены входа на * % if tp_price >= buy_price*0.2
